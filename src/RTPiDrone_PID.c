...
{
    static uint32_t pwmLast[4];
    for (int i=0; i<4; ++i) pwmLast[i] = pwm[i];
    for (int i=0; i<3; ++i) {
    ...
    /*
    pwm[0] += ( pid->output[1] - pid->output[0] + pid->output[2] );    //M0
    pwm[1] += (-pid->output[1] - pid->output[0] - pid->output[2] );    //M1
    pwm[2] += (-pid->output[1] + pid->output[0] + pid->output[2] );    //M2
    pwm[3] += ( pid->output[1] + pid->output[0] - pid->output[2] );    //M3
    */

    pwm[0] += (-pid->output[0] + pid->output[1]);    //M0
    pwm[1] += (-pid->output[0] - pid->output[1]);    //M1
    pwm[2] += ( pid->output[0] - pid->output[1]);    //M2
    pwm[3] += ( pid->output[0] + pid->output[1]);    //M3


    /*
        pwm[0] += ( pid->output[1]);    //M0
        pwm[1] += (-pid->output[1]);    //M1
        pwm[2] += (-pid->output[1]);    //M2
        pwm[3] += ( pid->output[1]);    //M3

        pwm[0] += (-pid->output[0]);    //M0
        pwm[1] += (-pid->output[0]);    //M1
        pwm[2] += ( pid->output[0]);    //M2
        pwm[3] += ( pid->output[0]);    //M3
    */
    for (int i=0; i<4; ++i) {
        if (pwm[i] >  power || pwm[i] < POWER_MIN) {
            pwm[i] = pwmLast[i];
        } else {
            pwmLast[i] = pwm[i];
        }
